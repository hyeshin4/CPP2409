...
// 목적지에 도달했는지 확인하는 함수
void handlePositionEffect(int map[][mapX], int &hp, int x, int y);
// 아이템-적과 상호작용하는 함수
void movePlayer(int map[][mapX], int &hp, int &x, int &y, int dx, int dy);
//사용자를 이동하는 함수
...
int main() {
    ...
    while (true) { // 사용자에게 계속 입력받기 위해 무한 루프
        ...
        // 위치 입력에 따른 상호작용
        if (user_input == "상") movePlayer(map, hp, user_x, user_y, 0, -1);
        else if (user_input == "하") movePlayer(map, hp, user_x, user_y, 0, 1);
        else if (user_input == "좌") movePlayer(map, hp, user_x, user_y, -1, 0);
        else if (user_input == "우") movePlayer(map, hp, user_x, user_y, 1, 0);
        // 지도 보여주기 함수 호출
...
void movePlayer(int map[][mapX], int &hp, int &x, int &y, int dx, int dy) {
    ...
    if (checkXY(newX, newY)) {
        ...
        handlePositionEffect(map, hp, x, y); // 아이템-적과 만났을 때 상호작용
    } else {
